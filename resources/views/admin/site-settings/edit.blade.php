...
<div class="ssw-wrap">
  <div class="ssw-head">
    <div>
      ...
      <h1 class="ssw-title">Site Settings Update Karein</h1>
      <p class="ssw-sub">{{ $setting->site_name ?? 'Settings' }} ki jaankari yahan se badlein — jo file naya nahi choose karenge wahi purani rahegi.</p>
    </div>
    <a href="{{ route('admin.site-settings.index') }}" class="ssw-back"><i class="fas fa-arrow-left"></i> List par wapas</a>
  </div>

  <form method="POST" action="{{ route('admin.site-settings.update',$setting) }}" enctype="multipart/form-data" id="sswForm" novalidate>
    @csrf @method('PUT')
    ...
    <div class="ssw-shell">
      ...
      <aside class="ssw-rail">
        <ol class="ssw-steps" id="sswSteps">
          ...
          <li class="ssw-step" data-step="2">
            ...
            <span class="ssw-step-text"><b>SEO &amp; Meta</b><small>Search visibility</small></span>
          </li>
          <li class="ssw-step" data-step="3">
            <span class="ssw-step-dot"><i class="fas fa-image"></i></span>
            <span class="ssw-step-text"><b>Logo &amp; Favicon</b><small>Core branding</small></span>
          </li>
          <li class="ssw-step" data-step="4">
            <span class="ssw-step-dot"><i class="fas fa-panorama"></i></span>
            <span class="ssw-step-text"><b>Banners</b><small>Homepage sliders</small></span>
          </li>
        </ol>
      ...
      <section class="ssw-panel">
        ...
        <div class="ssw-pane is-active" data-pane="1">
          <div class="ssw-pane-head"><span class="ssw-badge">01</span><div><h2>Site ki Pehchaan</h2><p>Naam aur tagline sabse pehle dikhte hain.</p></div></div>
          <div class="ssw-grid">
            <div class="ssw-field"><label>Site Name</label><input name="site_name" class="ssw-input" value="{{ old('site_name', $setting->site_name) }}"></div>
            <div class="ssw-field"><label>Tagline</label><input name="tagline" class="ssw-input" value="{{ old('tagline', $setting->tagline) }}"></div>
          </div>
        </div>

        <div class="ssw-pane" data-pane="2">
          <div class="ssw-pane-head"><span class="ssw-badge">02</span><div><h2>SEO &amp; Meta</h2><p>Google search results mein site kaise dikhegi.</p></div></div>
          <div class="ssw-grid">
            <div class="ssw-field"><label>Meta Title</label><input name="meta_title" class="ssw-input" value="{{ old('meta_title', $setting->meta_title) }}"></div>
            <div class="ssw-field"><label>Meta Keywords</label><input name="meta_keywords" class="ssw-input" value="{{ old('meta_keywords', $setting->meta_keywords) }}"></div>
            <div class="ssw-field ssw-span2"><label>Meta Description</label><textarea name="meta_description" class="ssw-input" rows="3">{{ old('meta_description', $setting->meta_description) }}</textarea></div>
          </div>
        </div>

        <div class="ssw-pane" data-pane="3">
          <div class="ssw-pane-head"><span class="ssw-badge">03</span><div><h2>Logo &amp; Favicon</h2><p>Nayi file choose karein tabhi purani replace hogi.</p></div></div>
        ...
        <div class="ssw-pane" data-pane="4">
          <div class="ssw-pane-head"><span class="ssw-badge">04</span><div><h2>Homepage Banners</h2><p>Slider mein chaaron banners is order mein chalenge.</p></div></div>
          <div class="ssw-grid ssw-grid-media">
            @for($i=1;$i<=4;$i++)
            <div class="ssw-upload">
        ...
        <div class="ssw-actions">
          ...
          <div class="ssw-actions-right">
            <span class="ssw-autosave"><i class="fas fa-shield-halved"></i> Bina naya upload kiye purani file safe rahegi</span>
...
<style>
  ...
  .ssw-panel{background:var(--ssw-glass);border:1px solid var(--ssw-line);border-radius:20px;padding:28px;box-shadow:0 20px 45px -30px rgba(30,27,46,.35);min-height:380px;display:flex;flex-direction:column;}
