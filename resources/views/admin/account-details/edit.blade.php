...
<div class="ssw-wrap">
  <div class="ssw-head">
    <div>
      ...
      <h1 class="ssw-title">Account Detail Update Karein</h1>
      <p class="ssw-sub">{{ $account->bank_name ?? 'Account' }} — {{ $account->account_holder_name ?? '' }} ki details yahan se badlein.</p>
    </div>
    <a href="{{ route('admin.account-details.index') }}" class="ssw-back"><i class="fas fa-arrow-left"></i> List par wapas</a>
  </div>

  <form method="POST" action="{{ route('admin.account-details.update',$account) }}" enctype="multipart/form-data" id="sswForm" novalidate>
    @csrf @method('PUT')
    ...
    <div class="ssw-shell">
      ...
      <section class="ssw-panel">
        ...
        <div class="ssw-pane is-active" data-pane="1">
          <div class="ssw-pane-head"><span class="ssw-badge">01</span><div><h2>Bank Details</h2><p>Yeh sabse zaroori jaankari hai — dhyan se badlein.</p></div></div>
          <div class="ssw-grid">
            <div class="ssw-field"><label>Account Holder Name <b class="ssw-req">*</b></label><input name="account_holder_name" class="ssw-input" value="{{ old('account_holder_name',$account->account_holder_name) }}" required></div>
            <div class="ssw-field"><label>Bank Name <b class="ssw-req">*</b></label><input name="bank_name" class="ssw-input" value="{{ old('bank_name',$account->bank_name) }}" required></div>
            <div class="ssw-field"><label>Account Number <b class="ssw-req">*</b></label><input name="account_number" class="ssw-input" value="{{ old('account_number',$account->account_number) }}" required></div>
            <div class="ssw-field"><label>Confirm Account Number</label><input name="confirm_account_number" class="ssw-input" value="{{ old('confirm_account_number',$account->confirm_account_number) }}"></div>
            <div class="ssw-field"><label>IFSC Code <b class="ssw-req">*</b></label><input name="ifsc_code" class="ssw-input" style="text-transform:uppercase" value="{{ old('ifsc_code',$account->ifsc_code) }}" required></div>
            <div class="ssw-field"><label>Branch Name</label><input name="branch_name" class="ssw-input" value="{{ old('branch_name',$account->branch_name) }}"></div>
            <div class="ssw-field">
              <label>Account Type</label>
              <select name="account_type" class="ssw-input">
                <option value="Saving" @selected($account->account_type=='Saving')>Saving</option>
                <option value="Current" @selected($account->account_type=='Current')>Current</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div class="ssw-pane" data-pane="2">
          <div class="ssw-pane-head"><span class="ssw-badge">02</span><div><h2>Digital Payments</h2><p>Nayi QR image choose karein tabhi purani replace hogi.</p></div></div>
          <div class="ssw-grid">
            <div class="ssw-field"><label>UPI ID</label><input name="upi_id" class="ssw-input" value="{{ old('upi_id',$account->upi_id) }}"></div>
            <div class="ssw-field"><label>UPI Number</label><input name="upi_number" class="ssw-input" value="{{ old('upi_number',$account->upi_number) }}"></div>
            <div class="ssw-field"><label>SWIFT Code</label><input name="swift_code" class="ssw-input" value="{{ old('swift_code',$account->swift_code) }}"></div>
            <div class="ssw-field"><label>MICR Code</label><input name="micr_code" class="ssw-input" value="{{ old('micr_code',$account->micr_code) }}"></div>
        ...
        <div class="ssw-pane" data-pane="3">
          ...
          <div class="ssw-grid">
            <div class="ssw-field"><label>GST Number</label><input name="gst_number" class="ssw-input" value="{{ old('gst_number',$account->gst_number) }}"></div>
            <div class="ssw-field"><label>PAN Number</label><input name="pan_number" class="ssw-input" value="{{ old('pan_number',$account->pan_number) }}"></div>
            <div class="ssw-field ssw-span2"><label>Payment Instructions</label><textarea name="payment_instructions" class="ssw-input" rows="4">{{ old('payment_instructions',$account->payment_instructions) }}</textarea></div>
          </div>
        </div>
          ...
          <div class="ssw-grid">
            ...
            <div class="ssw-field">
              ...
              <select name="show_on_payment_page" class="ssw-input">
                <option value="yes" @selected($account->show_on_payment_page=='yes')>Yes — Customers ko dikhega</option>
                <option value="no" @selected($account->show_on_payment_page=='no')>No — Sirf internal use</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div class="ssw-actions">
          ...
          <div class="ssw-actions-right">
            <span class="ssw-autosave"><i class="fas fa-shield-halved"></i> Bina naya upload kiye purana QR safe rahega</span>
            <button type="button" class="ssw-btn ssw-btn-primary" id="sswNext">Aage Badhein <i class="fas fa-arrow-right"></i></button>
            <button type="submit" class="ssw-btn ssw-btn-success d-none" id="sswSubmit"><i class="fas fa-check"></i> Update Karein</button>
          </div>
        </div>
